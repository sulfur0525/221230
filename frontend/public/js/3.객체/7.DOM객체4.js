...
selectbox.addEventListener('change', (e)=>{
	console.log(e)
	//1. 선택된 option
	e.currentTarget//이벤트 실행한 타켓[마크업] : selectbox
	console.log(e.currentTarget)
	
	let options = e.currentTarget.options //select의 모든 옵션들을 배열로 출력
	console.log(e.currentTarget.options)
	
	let index = e.currentTarget.options.selectedIndex //select 에서 선택한 인덱스 가져오기
...
timerbox.addEventListener('change', (e)=> {
	...
	if(e.currentTarget.checked){
		...
		//setInterval(함수명 , 시간(밀리초 = 1/1000초))
		//setInterval(()=>{},1000) : 1초마다 화살표 함수 실행
		//clearInterval() : Interval 초기화
			// let 변수명 = setInterval(()=>{},1000) : 1초마다 화살표 함수 실행
		timerId = setInterval(()=>{
			timer++;
